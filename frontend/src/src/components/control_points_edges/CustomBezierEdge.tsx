...
const CustomBezierEdge: React.FC<EdgeProps> = ({
  ...
  const initialControlPoints = (() => {
    ...
    const p1 = { x: sourceX + 50, y: sourceY };
    const p2 = { x: targetX - 50, y: targetY };
    const p3 = { x: targetX, y: targetY };
    return [1 / 3, 2 / 3].map((t, i) => {
      const { x, y } = getCubicBezierPoint(t, p0, p1, p2, p3);
      return { id: `cp-${i + 1}`, t, x, y };
    });
  })();
  ...
  const computeControlPoints = useCallback(() => {
    ...
    const newPoints = [1 / 3, 2 / 3].map((t, i) => {
      const { x, y } = getCubicBezierPoint(t, p0, p1, p2, p3);
      return { id: `cp-${i + 1}`, t, x, y };
    });
  ...
  const renderedControlPoints = controlPoints;
  ...
  const buildSmoothBezierPath = (
    ...
    const pts = [{ x: sourceX, y: sourceY }, ...controlPoints, { x: targetX, y: targetY }];
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      // Previous and next points for handle calculations
      const pMinus = pts[i - 1] || p0;
      const pPlus = pts[i + 2] || p1;

      const dx1 = (p1.x - pMinus.x) / 6;
      const dy1 = (p1.y - pMinus.y) / 6;

      const dx2 = (pPlus.x - p0.x) / 6;
      const dy2 = (pPlus.y - p0.y) / 6;

      const c1 = { x: p0.x + dx1, y: p0.y + dy1 };
      const c2 = { x: p1.x - dx2, y: p1.y - dy2 };
  ...
  const path = buildSmoothBezierPath(sourceX, sourceY, targetX, targetY, renderedControlPoints);
