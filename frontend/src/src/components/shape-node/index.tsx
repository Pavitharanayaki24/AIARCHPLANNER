...
import { type ShapeNode, ShapeType } from '../shape/types';
...
function ShapeNode({ id, selected, data }: NodeProps<ShapeNode>) {
  ...
  const darkerColor = darkenHexColor(color, 70);
  const lightColor = darkenHexColor(color, 30);
  ...
  function handlePlusClick(sourceHandle: string) {
   ...
   function renderPlusIcon(handleId: string, position: Position) {
    ...
    return (
      ...
      >
        <FaPlusCircle size={14} style={{color: isPlusHovered ? darkerColor : lightColor, cursor: "pointer"}}/>
      </div>
    );
  }
  
   // Utility to darken a hex color by a given percentage (0-100)
  function darkenHexColor(hex: string, percent: number) {
    ...
    // Convert back to hex
    const darkened = '#' + [r, g, b].map(x => x.toString(16).padStart(2, '0')).join('');
    return darkened;
  }
  ...
  const handleContainerResize = debounce(() => {
    ...
    setSortedNodes((prevNodes) => {
      let updatedNodes = prevNodes.map((n) =>
        n.id === currentNode?.id ? currentNode : n
      );

      attachNodesToContainer(currentNode, updatedNodes, (newNodes) => {
...
>
  ...
  <Shape
    ...
    width={width}
    height={height}
    fill={type === "dashed-rectangle" ? "transparent" : color}
    stroke={color}
    strokeDasharray={type === "dashed-rectangle" ? "6 3" : undefined}
    fillOpacity={type === "dashed-rectangle" ? 1 : 0.8}
    strokeWidth={type === "dashed-rectangle" ? 1 : 2}
  />
  ...
  >
    <NodeLabel
      nodeId={id}
      placeholder={type !== "dashed-rectangle" ? data.label : "Group"}
      type={type}
    />
  </div>
